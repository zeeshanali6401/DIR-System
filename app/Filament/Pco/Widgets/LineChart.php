...
class LineChart extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        $lastCaseDate = DIR::where('user_email', $userEmail)
            ->whereYear('case_date_time', $currentYear)
            ->max('case_date_time');
        $lastMonth = $lastCaseDate ? (new Carbon($lastCaseDate))->month : 0;
        $months = collect(range(1, $lastMonth))->map(function ($month) use ($currentYear, $userEmail) {
            return DIR::where('user_email', $userEmail)
                ->whereYear('case_date_time', $currentYear)
